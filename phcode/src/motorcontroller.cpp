...
AxisData MotorController::pitchData = {};
AxisData MotorController::rollData = {};

std::chrono::time_point<std::chrono::high_resolution_clock> MotorController::ptime1 = std::chrono::high_resolution_clock::now();
std::chrono::time_point<std::chrono::high_resolution_clock> MotorController::ptime2 = std::chrono::high_resolution_clock::now();
std::chrono::time_point<std::chrono::high_resolution_clock> MotorController::rtime1 = std::chrono::high_resolution_clock::now();
std::chrono::time_point<std::chrono::high_resolution_clock> MotorController::rtime2 = std::chrono::high_resolution_clock::now();
...
MotorController::MotorController(Alerts *alerts)
  : alerts(alerts),
    //xssm(NULL),
    hw(PORT),
    net(),
  ...
  pitch_profile.acc = 4000;
  pitch_profile.dec = 4000;
  roll_profile.acc = 4000;
  roll_profile.dec = 4000;
...
/*
  ...
  */
  //status_monitor = std::thread(&MotorController::__status_monitor, this);
	...
	// Setup params for controlling each axis
  	setupPitchControl();
	setupRollControl();
...
void MotorController::clearFaults(uint16_t ampID) {
  ...
  MotorController::catch_error(amp->ClearFaults());
...
void MotorController::setHomeLocation(uint16_t ampID) {
	...
	hc.velFast = 15; // uunits: platform deg
...
bool MotorController::saveConfigToFlash(uint16_t ampID) {
	Amp *amp = (ampID == 1) ? &pitch_amplifier : &roll_amplifier;

	if (amp == NULL) return false;

	const Error *err = amp->SaveAmpConfig();;
	// Read new amp mode to check that it worked
	std::cout << "Saved amp config for amp #" << ampID << std::endl;
...
void MotorController::__pitch_controller() {
  ...
  double lastPitchError = 0;//xssm->read<IDX_PITCH>();
...
/*
    ...
    float pitchError = 0;//xssm->read<IDX_PITCH>();  // vnsm->read<IDX_ROLL>();
	 float absPitchError = std::abs(pitchError);

	 //double roll_target_pos = roll
	 //roll_profile.pos = ROLL_MOVE_FACTOR * roll;

	double dampingFactor = absPitchError < X_DAMPING_LIMIT ? absPitchError / X_DAMPING_LIMIT : 1; //absPitchError < 1.98 ? absPitchError * absPitchError / 4. + .02 : 1;
...
void MotorController::__roll_controller() {
 ...
 // std::chrono::system_clock::time_point alarm_time;
  ...
  double lastRollError = 0;//xssm->read<IDX_ROLL>();
...
/*
    ...
    float rollError = 0;//xssm->read<IDX_ROLL>();  // vnsm->read<IDX_ROLL>();
	 float absRollError = std::abs(rollError);

	 //double roll_target_pos = roll
	 //roll_profile.pos = ROLL_MOVE_FACTOR * roll;

	double dampingFactor = absRollError < Y_DAMPING_LIMIT ? absRollError / Y_DAMPING_LIMIT : 1; //absRollError < 1.98 ? absRollError * absRollError / 4. + .02 : 1;
...
void MotorController::setupPitchControl() {

	// Home motor
	setHomeLocation(1);

	// Put motor controller into velocity mode
	setAmpMode(1, AMPMODE_CAN_VELOCITY);
	
	// Set initial gains
	pitchGains->K = 1.; //.1; //1;
	pitchGains->Kp = 18;//8; //69; //3;
	pitchGains->Ki = 0;//.1;//1; //.7; //1 //0; 
	pitchGains->Kd = 0;//6.9; //3.6; //6.9; //36;
   //working:: 1, 6.0, 1, 6.9

	// Set CAN profile params
	pitch_amplifier.SetProfileAcc(1000);
	pitch_amplifier.SetProfileDec(1000);

	// Activate pitch control
	pitchActive = true;


}

void MotorController::setupRollControl() {
	...
	// Set initial gains
	rollGains->K = 1.; //.1; //1;
	rollGains->Kp = 25;//8; //69; //3;
	rollGains->Ki = 0;//.1;//1; //.7; //1 //0; 
	rollGains->Kd = 0;//6.9; //3.6; //6.9; //36;
   //working:: 1, 6.0, 1, 6.9
...
void MotorController::correctInclination(float pitchError, float rollError) {

	if (pitchActive) {
		correctPitch(pitchError);
	}
	if (rollActive) {
		correctRoll(rollError);
	}

	pitchData.lastError = pitchError;
	rollData.lastError = rollError;

}

void MotorController::correctPitch(float pitchError) {
	
	ptime2 = std::chrono::high_resolution_clock::now();

	std::chrono::duration<double> elapsed = ptime2 - ptime1;
	double deltaT = elapsed.count();
	...
	// Update derivative and integral
	pitchData.derivative = (pitchError - pitchData.lastError) / deltaT; 

	pitchData.integral += pitchError * deltaT;

	// Calculate velocity update

	//double vel = dampingFactor * pitchGains->K * pitchError / (pitchGains->Kp + std::abs(pitchError));

	double vel = dampingFactor * pitchGains->K * 
		(pitchError * pitchGains->Kp 
		+ pitchData.integral * pitchGains->Ki 
		//+ std::min(0.0, pitchData.derivative / (pitchGains->Kd + pitchData.derivative)));
		+ pitchData.derivative * pitchGains->Kd);
	...
	if (std::abs(vel) > PITCH_MOTOR_MAX_VELOCITY) {
		vel = (vel < 0) ? -1 * PITCH_MOTOR_MAX_VELOCITY : PITCH_MOTOR_MAX_VELOCITY;
		if (std::abs(pitchGains->K * pitchGains->Ki * pitchData.integral) > PITCH_MOTOR_MAX_VELOCITY) {
			pitchData.integral = pitchData.integral > 0 ? PITCH_MOTOR_MAX_VELOCITY / (pitchGains->Ki * pitchGains->K) : -1 * PITCH_MOTOR_MAX_VELOCITY / (pitchGains->Ki * pitchGains->K);
		}
	}
	...
	if (this->printPitch) {
		...
		std::cout << ", Dir: " << setw(4) << pitch_profile.dir;
		std::cout << ", Pitch Velocity: " << setw(8) << vel; //pitch_profile.vel;
		std::cout << ", Pitch Integral: " << setw(8) << pitchData.integral;
		std::cout << ", DeltaT: " << setw(8) << deltaT;;
		std::cout << std::flush;
...
void MotorController::correctRoll(float rollError) {

	rtime2 = std::chrono::high_resolution_clock::now();

	std::chrono::duration<double> elapsed = rtime2 - rtime1;
	double deltaT = elapsed.count();
	...
	// Update derivative and integral
	rollData.derivative = (rollError - rollData.lastError) / deltaT; 

	rollData.integral += rollError * deltaT;

	// Calculate velocity update

	//double vel = dampingFactor * rollGains->K * rollError / (rollGains->Kp + std::abs(rollError));

	double vel = dampingFactor * rollGains->K * 
		(rollError * rollGains->Kp 
		+ rollData.integral * rollGains->Ki 
		//+ std::min(0.0, rollData.derivative / (rollGains->Kd + rollData.derivative)));
		+ rollData.derivative * rollGains->Kd);
	...
	if (std::abs(vel) > ROLL_MOTOR_MAX_VELOCITY) {
		vel = (vel < 0) ? -1 * ROLL_MOTOR_MAX_VELOCITY : ROLL_MOTOR_MAX_VELOCITY;
		if (std::abs(rollGains->K * rollGains->Ki * rollData.integral) > ROLL_MOTOR_MAX_VELOCITY) {
			rollData.integral = rollData.integral > 0 ? ROLL_MOTOR_MAX_VELOCITY / (rollGains->Ki * rollGains->K) : -1 * ROLL_MOTOR_MAX_VELOCITY / (rollGains->Ki * rollGains->K);
		}
	}

	roll_profile.vel = std::abs(vel); //velocities[0] + velocities[1] + velocities[2]) / 3;
	...
	if (this->printRoll) {
		...
		std::cout << "\r" << "> Roll: " << setw(8) << rollError;
		std::cout << ", Dir: " << setw(4) << roll_profile.dir;
		std::cout << ", Roll Velocity: " << setw(8) << vel; //pitch_profile.vel;
		std::cout << ", Roll Integral: " << setw(8) << rollData.integral;
		std::cout << ", DeltaT: " << setw(8) << deltaT;; std::cout << std::flush;
