...
This function increases the reference count associated with the object and 
can be used if a reference ID needs to be copied and stored for use by 
another class.

For each call to GrabRef, there should be a corresponding call to 
RefObj::ReleaseRef to release the reference when it's no longer needed.

@param id The reference ID of the object.

@return true if the reference count was successfully increased.  False if
an invalid reference ID was passed.
 */
/***************************************************************************/
bool RefObj::GrabRef( uint32 id )
{
   int32 refID = id-1;
   ...
   // Just return false if the reference is no longer valid
   if( !ri ) return false;
