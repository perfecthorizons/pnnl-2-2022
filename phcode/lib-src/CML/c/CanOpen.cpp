...
// TODO - need to test timing master support
...
const Error *CanOpen::DetachNode( Node *n )
{
   ...
   // Disable node guarding.  This removes the node from the 
   // linked list maintained by the node guarding thread.
   guard.SetNodeGuard( GetCoInfo( n ), GUARDTYPE_NONE, 0, 0 );
...
@param timeout Max time to wait for the SDO to be sent.
@return A pointer to an error object, or NULL on success.
*/
/***************************************************************************/
...
void CanOpen::run( void )
{
   ...
   while( canRef )
   {
      ...
      {
         cml.Error( "Error frame received.  Total of %d error frames seen since last clear\n", ++errorFrameCt );
         continue;
      }
...
// Remove this node from the linked list of node info objects maintained by the 
// node guarding thread.  Note that any time this function is called the node 
// guarding thread's mutex is locked.
