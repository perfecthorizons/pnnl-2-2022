...
public:
   ...
   const Error *NewData( void )
   {
      ...
      // If it is, just ignore this since it contains invalid data
      if( !getWKT() )
         return 0;
...
class ALstatDG: public FPRD
{
   uint32 nodeRef;
public:
   ALstatDG( Node *n, int16 nodeID ): FPRD( nodeID, 0x0130, 2 )
   { 
      nodeRef = n->GrabRef();
   }

   // Check the node's state to see if it changed
   const Error *NewData( void )
   {
      // I store the expected state in the index byte since the drive doesn't touch that
      uint8 expected = getNdx();

      // If this is zero, just quit.  I'm ignoring the response
      if( !expected ) return 0;

      // If for some reason the read wasn't processed by the drive, just ignore it
      if( !getWKT() ) return 0;

      // Compare the value I read to the expected value and don't do anything if they match
      uint16 state = getData16u();
      if( state == expected ) return 0;

      // Get a pointer to the node and just quit if it's no longer valid
      Node *n = (Node *)RefObj::LockRef( nodeRef );
      if( !n ) return 0;

      // Inform the node of the state change
      n->SetState( FindNodeState(state) );

      // Save the new state so I don't keep updating the node
      setNdx( state );
   ...
   // Turn off processing of this status.  We turn this off when changing state
   void Disable( void )
   {
      setNdx(0);
   }

   // Enable processing and set the expected state
   void Enable( uint8 state )
   {
      setNdx( state );
   }
};
...
public:
   ...
   // Datagram used to read link AL status
   ALstatDG alStatDG;
   ...
   EtherCatNodeInfo( Node *node, int nodeID ) : alStatDG( node, nodeID )
   {
      id = nodeID;
      mbCount = 0;
      foePacket = 0;
...
const Error *EtherCAT::AttachNode( Node *n )
{
   ...
   EtherCatNodeInfo *ni = new EtherCatNodeInfo( n, id );
   SetNodeInfo( n, ni );
...
const Error *EtherCAT::FindSyncMgrCfg( Node *n, bool boot )
{
   ...
   while( addr < 0x1000 )
   {
      ...
      if( err )
      {
         cml.Debug( "Error reading EEPROM address 0x%04x - %s\n", addr, err->toString() );
         return err;
      }
      addr += 2;
      ...
      if( cat == 0xffff )
      {
         cml.Debug( " - EEPROM read at 0x%04x returned 0xFFFF (end of category info)\n", addr );
         return 0;
      }

      cml.Debug( " - EEPROM 0x%04x - category %d, len %d\n", addr, cat, len );
      ...
      if( !len )
      {
         cml.Debug( " - Aborting EEPROM read due to section of length 0 (invalid)\n" );
         return 0;
      }
   ...
   if( cat != 41 )
   {
      cml.Debug( " - Never found sync manager info in EEPROM, quitting\n" );
      return 0;
   }
...
void EtherCAT::CycleThreadFunc( void )
{
   ...
   while( !stopSem )
   {
      ...
      for( int n=0; n<nodeCt; n++ )
      {
         ...
         // Add a PDO to poll the node state
         AddToFrame( &frame, &ni->alStatDG );
...
const Error *EtherCAT::SetNodeAlState( Node *n, uint16 state )
{
   ...
   // Turn off cyclic polling of the node state
   ni->alStatDG.Disable();
...
const Error *EtherCAT::MailboxTransfer( Node *n, uint16 len, uint16 *ret, Timeout timeout )
{
   ...
   return MailboxTransfer( n, ni->syncBuff[ SM_RXMBX ], len, ni->syncBuff[ SM_TXMBX ], ret, ni->syncLen[SM_TXMBX], timeout, false );
}
...
/**
  ...
  @param n     The node to access.
  @param send  Buffer holding the data to send to the node
  @param len   The number of bytes of data to send
  @param recv  Buffer where the received data will be returned
  @param ret   returns the actual number of bytes returned.
  @param max   Maximum number of bytes to return
  @param timeout The maximum time to wait for a response.
  @param getMtx  True if the mailbox mutex should be acquired by this function.  False if the mutex is already held by the calling function.
  @return An error pointer on failure, or null on success.
*/
const Error *EtherCAT::MailboxTransfer( Node *n, void *send, uint16 len, void *resp, uint16 *ret, uint16 max, Timeout timeout, bool getMtx )
{
   EtherCatNodeInfo *ni = GetEcatInfo( n );
   if( !ni ) return &EtherCatError::NodeNotInit;

   // make sure the send length is reasonable
   if( (len > ni->syncLen[SM_RXMBX]) || (len<6) )
      return &CanOpenError::BadParam;

   // Limit the maximum return size to the mailbox size
   if( max > ni->syncLen[ SM_TXMBX ] )
      max = ni->syncLen[ SM_TXMBX ];
   ...
   if( !ok )
   {
      cml.Debug( "Failed to write to mailbox of node %d, wkt never got updated.\n", ni->id );
      return &EtherCatError::NoResponse;
   }
   ...
   while(true)
   {
      ...
      if( retLen > max )
      {
         cml.Warn( "Node %d mailbox read returned %d bytes.  Truncated to %d to fit in buffer\n", ni->id, retLen, max );
         retLen = max;
      }
      ...
      *ret = retLen;
      ...
      if( (retLen >= 6) && !(ptr[5]&0x0f) )
      {
         ...
         int16 err = bytes_to_int16( &ptr[8] );
         switch( err )
         {
