...
@param saveProfile If true, the device profile parameters will be saved.
			       If false, the device profile parameters will not be saved.
@return An error object
*/
/***************************************************************************/
const Error *Node::SavePDOMapping(bool saveProfile)
{
	uint32 i;
	uint16 iNet;
	// Set bit 3 on the misc amplifier options register (0x2420) to enable
	// saving PDO configuration to a file in the CVM file system.
	const Error *err = sdo.Upld32(0x2420, 0, i);
	if (err) return err;

	i = i | 0x8;

	err = sdo.Dnld32(0x2420, 0, i);
	if (err) return err;

	// Set object 0x21B3 sub index 0 bits 8 and 9 for CAN or bit 8
	// for EtherCAT.
	err = sdo.Upld16(0x21B3, 0, iNet);
	if (err) return err;

	// Check if the network is CANopen or EtherCAT
	if (GetNetworkType() == NET_TYPE_CANOPEN)
		iNet = iNet | 0x300;
	else
		iNet = iNet | 0x100;

	err = sdo.Dnld16(0x21B3, 0, iNet);
	if (err) return err;


	// Use object 0x1010 to save to flash
	// Writing to sub-index 1 saves all objects to flash.
	// Writing to sub-index 2 saves communication parameters (0x1000-0x1FFF).
	// This range includes all PDO mappings.
	// Writing to sub-index 3 saves device profile parameters (0x6000-0x9FFF).
	// Writing to sub-index 4 saves manufacturer specific parameters (0x2000-0x5FFF).
	err = sdo.Dnld32(0x1010, 2, 0x65766173);
	if (err) return err;

	// Save the device profile parameters.
	if (saveProfile)
	{
		err = sdo.Dnld32(0x1010, 3, 0x65766173);
		if (err) return err;
	}
	return 0;
}
...
void Node::SetState( NodeState newState )
{
   ...
   if( old != newState )
   {
      cml.Debug( "Node %d state changed from %d to %d\n", GetNodeID(), old, newState );
