...
/***************************************************************************/
/**
Get the jerk value used during trajectory aborts. If this is 0, then the abort will
be calculated without any jerk limits.
...
*/
/***************************************************************************/
const Error *Amp::GetTrajectoryJrkAbort(uunit &value)
{
	uint32 j;
	const Error *err = Upld32(OBJID_TRJ_JRK_ABORT, 0, j);
	value = JrkLoad2User(j);
	return err;
}

/***************************************************************************/
/**
Set the jerk value used during trajectory aborts. If this is 0, then the abort will
be calculated without any jerk limits.

This parameter is specified in "user units".  See Amp::SetCountsPerUnit for details.

@param value The jerk value to be used.
@return A pointer to an error object, or NULL on success
*/
/***************************************************************************/
const Error *Amp::SetTrajectoryJrkAbort(uunit value)
{
	return Dnld32(OBJID_TRJ_JRK_ABORT, 0, (int32)value);
}

/***************************************************************************/
/**
Get the servo loop configuration. This parameter allows various parts of the drive
servo loops to be enabled/disabled.

@param value variable that will store the returned value
@return A pointer to an error object, or NULL on success
*/
/***************************************************************************/
const Error *Amp::GetServoLoopConfig(int32 &value)
{
	return Upld32(OBJID_SERVO_LOOP_CFG, 0, value);
}

/***************************************************************************/
/**
Set the servo loop configuration. This parameter allows various parts of the drive
servo loops to be enabled/disabled.

@param value The servo config value to be used.
@return A pointer to an error object, or NULL on success
*/
/***************************************************************************/
const Error *Amp::SetServoLoopConfig(int32 value)
{
	return Dnld32(OBJID_SERVO_LOOP_CFG, 0, value);
}
...
const Error *Amp::ClearFaults( void )
{
   ...
   // If we aren't in CANopen mode, then we need to clear faults by
   // writing to the fault object.  The control word is only used in CANopen modes
   if( !isCanMode( lastMode ) )
   {
      err = Dnld32( OBJID_FAULTS, 0, (uint32)0xffffffff );
      if( err ) return err;
   ...
   // status is consistant with the drive's.
   EventNone e( AMPEVENT_FAULT );
   WaitEvent( e, 100 );
...
const Error *Amp::SetControlWord( uint16 value )
{
   ...
   // For CANopen networks, we use an SDO to update the control word
   const Error *err;
   if( GetNetworkType() == NET_TYPE_CANOPEN )
      err = Dnld16( OBJID_CONTROL, 0, value );

   // For EtherCAT, we use a PDO.  This is primarily done to ensure that we can 
   // update the control word on multiple drives in a Linkage at the same time.
   else
   {
      RefObjLocker<EtherCAT> ecat( GetNetworkRef() );
      xmitCtrlPDO( value );
      err = ecat->WaitCycleUpdate( 20 * ecat->GetCyclicPeriod() );
   }
