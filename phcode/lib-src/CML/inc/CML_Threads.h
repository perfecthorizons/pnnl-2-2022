...
public:
   ...
   /// Return the current time in microsecond units.  The value returned may be 
   /// offset by a consistent, but arbitrary value.  This makes it useful for 
   /// checking relative times, but not useful for absolute time calculations.
   /// @return The time in microsecond units
...
public:
   ...
   /// Lock the passed mutex and keep a pointer to it.  The mutex will be unlocked in 
   /// the destructor if not explicitely unlocked first.  If another mutex was previously
   /// locked then it will be forgotten.
   /// @param m The mutex to lock.
   void Lock( Mutex &m )
   {
      mPtr = &m;
