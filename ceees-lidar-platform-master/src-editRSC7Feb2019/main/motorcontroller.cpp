...
MotorController::MotorController(VNSM *vnsm, Alerts *alerts)
  : alerts(alerts),
    vnsm(vnsm),
    hw(PORT),
    net(),
...
void MotorController::__pitch_controller() {
  ...
  while (controller_continue) {
    ...
    float pitch = vnsm->read<IDX_PITCH>(); //vnsm->read<IDX_PITCH>();
    ...
    double vel = std::min(PITCH_MOTOR_SENSITIVITY * std::abs(pitch), PITCH_MOTOR_MAX_VELOCITY);
    uunit move_len = vel / 100 * 1.1; //100 hz rate. Should use PITCH_CONTROLLER_RATE...
    if (pitch >= 0)
      move_len *= -1;
...
//    pitch_amplifier.DoMove(pitch_profile);
    ...
    alerts->diagnostic("READ pitch=%f; SET pitch_velocity=%f", pitch, pitch_profile.vel);
...
*/
    ...
    auto roll = vnsm->read<IDX_ROLL>();  // vnsm->read<IDX_ROLL>();
    ...
    uunit move_len = vel / 100 * 1.1; //100 hz rate. Should use ROLL_CONTROLLER_RATE...
    if (roll < 0)
      move_len *= -1;
...
//    roll_amplifier.DoMove(roll_profile);
    ...
    alerts->diagnostic("READ roll=%f; SET roll_velocity=%f", roll, roll_profile.vel);
    std::this_thread::sleep_until(alarm_time);
  }
}
