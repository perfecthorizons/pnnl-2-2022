...
struct join_tuples<std::tuple<LHS...>, std::tuple<RHS...>> {
  typedef std::tuple<LHS..., RHS...> type;
};

template<typename T, unsigned N>
struct generate_tuple {
  typedef typename generate_tuple<T, N/2>::type left;
  typedef typename generate_tuple<T, N/2 + N%2>::type right;
...
private:
  ...
  std::string __join_tuple(TupleType &tuple, std::string glue, std::index_sequence<I...>) {
    std::string output;
    (..., (output += std::get<I>(tuple) + (I == 0 ? glue : "")));
    return output;
  }
public:
