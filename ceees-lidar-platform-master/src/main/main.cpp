 ...
 */
void __gain_tuning_shell(MotorController* mc, XSSM *xssm, Alerts *alerts) {
...
******************/
	...
	//printBrakeStatus();
	MotorController m(1);
	cout << m.getBrakeStatus() << endl;
	MotorController n(2);
	cout << n.getBrakeStatus() << endl;
	*/
  ...
  while (!done) {
	...
	/* First attempt at homing --> having the user manually determine if the unit was centered
										--> now use hardstop methos
	done = 0;
	while (!done) {
		cout << "Is the device homed yet (centered relative to its base -- NOT the horizon)? (1=yes; 2=no): ";
		int choice = 0;
		while (!(cin>>choice)) {
			cin.clear();
			cin.ignore(numeric_limits<streamsize>::max(), '\n');
		...
		switch (choice) {
			...
			case 2:
				...
				cout << "Please manually move each motor until it is centered on its axis:\n";
				cout << "Pitch axis:\n";
				manualMove(1);
				cout << "Roll axis:\n";
				manualMove(2);
