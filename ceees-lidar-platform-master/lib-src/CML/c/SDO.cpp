...
public:
   ...
   {
      // If the last block was the end of data, then I'm expecting the next
      // message to be a block upload finished indication.
      if( done && (lastBlk==0) && ((frame.data[0] & 0xE3)==0xC1) )
      {
         done = 2;
         for( int i=0; i<8; i++ )
...
const Error *SDO::BlockUpld( int16 index, int16 sub, int32 &size, byte *data )
{
   ...
   while( 1 )
   {
      ...
      // Check for abort
      if( blkRcvr.lastFrame[0] == 0x80 )
         return getAbortRcvdErr( blkRcvr.lastFrame );

      // See if the block upload has finished
      if( blkRcvr.IsFinished() )
         break;
...
const Error *SDO::SendAbort( const Error *err )
{
   switch( err->GetID() )
   {
      case CMLERR_CanOpenError_SDO_BadMsgRcvd: SendAbort( SDO_ABORT_BAD_SCS,     err ); break;
      case CMLERR_CanOpenError_SDO_BadMuxRcvd: SendAbort( SDO_ABORT_GENERAL_ERR, err ); break;
      case CMLERR_SDO_Error_Togglebit:         return SendAbort( SDO_ABORT_TOGGLEBIT,   err );
      default:                                 return SendAbort( SDO_ABORT_TIMEOUT,     err );
   }

   // On CANopen networks, certain errors can be caused if the node incorrectly sends a 
   // repeated SDO response.  In those cases it's possible for the master & slave to become
   // out of sync resulting in multiple SDO errors occuring in row even though there was only
   // a single bad message on the bus.  This can be avoided by simply delaying briefly when
