...
// local data
static double ctrToMSec = -1;
static double ctrToUSec;
...
Thread::Thread( void )
{
   ...
   // If not already done, find scalers to convert performance counter
   // values into millisecond and microsecond values
   if( ctrToMSec < 0 )
   {
      LARGE_INTEGER li;
      if( QueryPerformanceFrequency( &li ) )
      {
         ctrToMSec = 1000.0 / li.QuadPart;
         ctrToUSec = ctrToMSec * 1000;
      }
   }
...
uint32 Thread::getTimeMS( void )
{
   ...
   return (uint32)(li.QuadPart * ctrToMSec);
}
...
uint32 Thread::getTimeUS( void )
{
   LARGE_INTEGER li;
   QueryPerformanceCounter( &li );
   return (uint32)(li.QuadPart * ctrToUSec);
}
...
Mutex::Mutex( void )
{
   ...
   if( !data )
      cml.Error( "Mutex creation failed\n" );
}
...
const Error *Mutex::Lock( void )
{
   if( !data )
   {
      cml.Error( "Attempting to lock a mutex with no data!\n" );
...
const Error *Mutex::Unlock( void )
{
   if( !data )
   {
      cml.Error( "Attempting to unlock a mutex with no data!\n" );
      return &ThreadError::Alloc;
   }
