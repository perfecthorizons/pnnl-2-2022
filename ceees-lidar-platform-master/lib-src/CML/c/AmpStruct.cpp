...
const Error *Amp::SetSoftLimits( SoftPosLimit &cfg )
{
   ...
   int32 m = PosUser2Mtr(cfg.motorPosWrap);
...
const Error *Amp::GetSoftLimits( SoftPosLimit &cfg )
{
   ...
   cfg.motorPosWrap = PosMtr2User( m );
   cfg.loadPosWrap = PosLoad2User( l );
...
const Error *Amp::GetIoConfig( AmpIoCfg &cfg )
{
	...
	if (!err && CheckFeature(FEATURE_IOPULLUP32))
		err = GetIoPullup32(cfg.inPullUpCfg32);
	if (!err && CheckFeature(FEATURE_IO_OPTIONS))
...
@param cfg A structure that holds the encoder error configuration settings.
@return A pointer to an error object, or NULL on success
*/
/***************************************************************************/
const Error *Amp::SetEncoderErrorConfig( EncoderErrorConfig &cfg )
{
	const Error *err = 0;

	if (CheckFeature(FEATURE_ENCODER_ERROR_CONFIGURATION))
	{
		err = Dnld32(OBJID_ENCFILT_CFG, 0, cfg.encoderErrorConfig);
	}
...
*/
/***************************************************************************/
const Error *Amp::GetEncoderErrorConfig( EncoderErrorConfig &cfg )
{
	const Error *err = Upld32(OBJID_ENCFILT_CFG, 0, cfg.encoderErrorConfig);
	return err;
}

/***************************************************************************/
/**
Configure the amplifier's special firmware. These will use serial binary commands
since there are no corresponding CAN objects.

@param cfg A structure that holds the configuration settings.
@return A pointer to an error object, or NULL on success
*/
/***************************************************************************/
const Error *Amp::SetSpecialFirmwareConfig( AmpConfig &cfg )
{
	const Error *err = 0;
	uint8 size = 1;
	uint8 max = 2;

	if (CheckFeature(FEATURE_SPECIAL_KM_FREQUENCY))
	{
		uint16 data[2];
		size = 2;
		max = 2;
		uint16 frequency = cfg.io.inCfg[0];
		data[0] = 0x1078;
		data[1] = frequency;
		err = SerialCmd(0x0D, size, max, data);

	}

	if (!err && CheckFeature(FEATURE_SPECIAL_KM_MAX_TEMP))
	{
		uint16 data[2];
		size = 2;
		max = 2;
		...
		data[0] = 0x1079;
		data[1] = cfg.io.inCfg[1];
		err = SerialCmd(0x0D, size, max, data);
	}

	if (!err && CheckFeature(FEATURE_SPECIAL_KM_MAX_TEMP_2))
	{
		uint16 data[2];
		size = 2;
		max = 2;
		...
		data[0] = 0x107A;
		data[1] = cfg.io.inCfg[2];
		err = SerialCmd(0x0D, size, max, data);
	}

	if (!err && CheckFeature(FEATURE_SPECIAL_KM_DWELL_TIME))
	{
		uint16 data[2];
		size = 2;
		max = 2;
		uint16 debounce = cfg.io.inDebounce[0];
		data[0] = 0x10F0;
		data[1] = debounce;
		err = SerialCmd(0x0D, size, max, data);
	}

	if (!err && CheckFeature(FEATURE_SPECIAL_KM_SOLE_CONT_VOLTAGE))
	{
		uint16 data[2];
		size = 2;
		max = 2;
		data[0] = 0x107B;
		data[1] = cfg.io.inCfg[3];;
		err = SerialCmd(0x0D, size, max, data);
	}

	if (!err && CheckFeature(FEATURE_SPECIAL_KM_SOLE_INIT_VOLTAGE))
	{
		uint16 data[2];
		size = 2;
		max = 2;
		data[0] = 0x107C;
		data[1] = cfg.io.inCfg[4];
		err = SerialCmd(0x0D, size, max, data);
	}
...
/***************************************************************************/
/**
Reads the amplifier's special firmware. These will use serial binary commands
since there are no corresponding CAN objects.
...
*/
/***************************************************************************/
const Error *Amp::GetSpecialFirmwareConfig( AmpConfig &cfg )
{
	const Error *err = 0;
	uint8 size = 1;
	uint8 max = 2;
	...
	if (CheckFeature(FEATURE_SPECIAL_KM_FREQUENCY))
	{
		...
		data[0] = 0x1078;
		data[1] = frequency;
		err = SerialCmd(0x0C, size, max, data);
...
const Error *Amp::SetPwmInConfig( PwmInConfig &cfg )
{
   ...
   if( !err && CheckFeature( FEATURE_PWMIN_DEADBAND ) )
	  err = Dnld16(OBJID_PWMIN_DBAND, 0, cfg.deadBand);

   // PWM input frequency was added to CANopen interface in version 4.56
   if( ( !err && CheckFeature( FEATURE_PWMIN_FREQ ) ) &! CheckFeature( FEATURE_PWMIN_DEADBAND ) )
...
const Error *Amp::GetPwmInConfig( PwmInConfig &cfg )
{
   ...
   if( !err && CheckFeature( FEATURE_PWMIN_DEADBAND ) )
	  err = Upld16( OBJID_PWMIN_DBAND, 0, cfg.deadBand );
   if( ( !err && CheckFeature( FEATURE_PWMIN_FREQ ) ) &! CheckFeature( FEATURE_PWMIN_DEADBAND ) )
...
const Error *Amp::GetAmpConfig( AmpConfig &cfg )
{
   ...
   if ( !err && CheckFeature( FEATURE_TRAJECTORY_JERK_ABORT ) )
   {
	   uunit temp = 0;
	   err = GetTrajectoryJrkAbort(temp);
	   cfg.trjJrkAbort = temp;
   }

   if ( !err && CheckFeature( FEATURE_SERVO_LOOP_CONFIG ) )
	   err = GetServoLoopConfig(cfg.servoConfig.servoLoopConfig);

   if (!err && CheckFeature(FEATURE_ENCODER_ERROR_CONFIGURATION))
	   err = GetEncoderErrorConfig(cfg.errFilterCfg);
...
const Error *Amp::SetAmpConfig( AmpConfig &cfg )
{
   ...
   if( !err && CheckFeature(FEATURE_TRAJECTORY_JERK_ABORT)) 
	   err = SetTrajectoryJrkAbort( cfg.trjJrkAbort );
   if( !err && CheckFeature(FEATURE_SERVO_LOOP_CONFIG)) 
	   err = SetServoLoopConfig( cfg.servoConfig.servoLoopConfig );
   if( !err ) err = SetTrackingWindows( cfg.window );
   if( !err ) err = SetSoftLimits( cfg.limit );
   if( !err ) err = SetIoConfig( cfg.io );
   if( !err ) err = SetEncoderErrorConfig( cfg.errFilterCfg );
   if( !err ) err = SetSpecialFirmwareConfig(cfg);
