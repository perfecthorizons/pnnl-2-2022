...
enum AMP_TRACE_TRIGGER
{
   ...
   /// Treat the trigger level as a bit mask which selects one or more
   /// bits on the amplifier's capture status register.  The trigger occurs
   /// as any of the selected bits are set.  Note that this trigger type
   /// does not use a trace variable.
   ...
   /// Treat the trigger level as a bit mask which selects one or more
   /// bits on the selected trace variable.  The trigger occurs as soon
   /// as any of the selected bits change state.
...
enum AMP_FEATURE
{
   ...
   // PWM input deadband was moved to 0x232B on plus products.
   FEATURE_PWMIN_DEADBAND,

   // Trajectory jerk abort.
   FEATURE_TRAJECTORY_JERK_ABORT,

   // Servo loop configuration
   FEATURE_SERVO_LOOP_CONFIG,

   // Does the amp support encoder error configuration
   FEATURE_ENCODER_ERROR_CONFIGURATION,

   // KM special firmware 0x78
   FEATURE_SPECIAL_KM_FREQUENCY,

   // KM special firmware 0x79
   FEATURE_SPECIAL_KM_MAX_TEMP,
   
   // KM special firmware 0x7A
   FEATURE_SPECIAL_KM_MAX_TEMP_2,

   // KM special firmware 0xf0
   FEATURE_SPECIAL_KM_DWELL_TIME,

   // KM special firmware 0x7B
   FEATURE_SPECIAL_KM_SOLE_CONT_VOLTAGE,

   // KM special firmware 0x7C
   FEATURE_SPECIAL_KM_SOLE_INIT_VOLTAGE
